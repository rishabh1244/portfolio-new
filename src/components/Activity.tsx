...
const SpotifyIcon = () => (
  <svg className={styles.activityIcon} viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg">
...
const DiscordIcon = () => (
  <svg className={styles.activityIcon} viewBox="0 0 32 32" fill="none" xmlns="http://www.w3.org/2000/svg">
...
const Activity = () => {
  ...
  const hasAnyContent = hasSpotify || activities.length > 0

  return (
    <section className="content-section section-border" id="activity">
      {/* -- To remove heading + divider, comment out this block -- */}
      <div className="sectionHeading"><h2>Activity</h2></div>
      <div className={styles.divider} />
      <div className={styles.activityBody}>
        {!data && (
          <div className={styles.empty}>Unable to load activity data.</div>
        )}
          ...
          <div className={styles.activitiesGrid}>
            {isPlaying && spotify && (
              <div className={styles.activityItem}>
                ...
                <div className={styles.activityContent}>
                  <div className={styles.activityLabel}>NOW PLAYING</div>
                  <div className={styles.activityName}>{spotify.song}</div>
                  <div className={styles.activitySub}>{spotify.artist}</div>
                </div>
              </div>
            )}

            {!isPlaying && hasSpotify && spotify && (
              <div className={styles.activityItem}>
                <SpotifyIcon />
                <div className={styles.activityContent}>
                  <div className={styles.activityLabel}>LAST PLAYED</div>
                  <div className={styles.activityName}>{spotify.song}</div>
                  <div className={styles.activitySub}>{spotify.artist}</div>
                </div>
              </div>
            ...
            {activities.map((act, i) => {
              const elapsed = act.timestamps?.start
                ? now - act.timestamps.start
                : null
              const label = activityTypeLabel[act.type] ?? 'Activity'

              return (
                <div className={styles.activityItem} key={`${act.name}-${i}`}>
                  <DiscordIcon />
                  <div className={styles.activityContent}>
                  ...
                  {act.assets?.large_text && <div className={styles.activitySub}>{act.details}</div>}
                  </div>
                </div>
              )
            })}
